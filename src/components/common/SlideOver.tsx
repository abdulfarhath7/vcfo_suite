...
export function SlideOver({ open, onOpenChange, title, description, children, footer }: Props) {
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex w-full flex-col border-l border-blue-100 p-0 sm:max-w-lg">
        <div className="accent-bar absolute left-0 top-0 h-full" aria-hidden />
        <SheetHeader className="border-b border-border bg-blue-50/40 px-6 py-4 pl-7">
        ...
        {footer && (
          <div className="border-t border-border bg-blue-50/30 px-6 py-3">{footer}</div>
        )}
      </SheetContent>
    </Sheet>
  );
