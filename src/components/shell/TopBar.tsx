...
export function TopBar() {
  const { user, signOut, setCommandOpen, sidebarCollapsed } = useApp();
  const { openMobile } = useShellNav();
  const router = useRouter();
  const isApple = useModShortcut();
  const homeHref = user ? roleHomePath(user.role) : "/";
  /* Wordmark lives in the sidebar when expanded; keep it here on mobile
     and when the desktop nav is icons-only so “VCFO Suite” never disappears. */
  const showWordmarkInBar = sidebarCollapsed;
